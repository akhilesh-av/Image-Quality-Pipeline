...
class ImageQualityPipeline:
    ...
    def step_12_border_removal(self):
        """Crop unnecessary whitespace or background edges with safer parameters"""
        try:
            image_cv = cv2.cvtColor(np.array(self.image), cv2.COLOR_RGB2BGR)
            ...
            # Add dilation to connect nearby content
            ...
            if contours:
                ...
                min_area = (image_cv.shape[0] * image_cv.shape[1]) * 0.01  # At least 1% of image area
                significant_contours = [c for c in contours if cv2.contourArea(c) > min_area]

                if significant_contours:
                    # Combine all significant contours
                    combined = np.vstack(significant_contours)
                    x, y, w, h = cv2.boundingRect(combined)

                    # Add safety margin (5% of each dimension)
                    margin_x = int(w * 0.05)
                    margin_y = int(h * 0.05)
                    x = max(0, x - margin_x)
                    y = max(0, y - margin_y)
                    w = min(image_cv.shape[1] - x, w + 2*margin_x)
                    h = min(image_cv.shape[0] - y, h + 2*margin_y)

                    # Only crop if we're keeping substantial content
                    if w > image_cv.shape[1] * 0.1 and h > image_cv.shape[0] * 0.1:
                        cropped = image_cv[y:y+h, x:x+w]
                        self.image = Image.fromarray(cropped)
                        message = f'Cropped to remove borders: {w}x{h}'
                    else:
                        message = 'Content area too small - skipped cropping'
                else:
                    message = 'No significant content found - skipped cropping'
            else:
                message = 'No contours found - skipped cropping'
            ...
            self.report['flags'].append('Border removal failed')
    ...
    def step_13_shadow_cleaning(self):
        ...
        try:
            ...
            if len(image_cv.shape) > 2:  # Color image
                gray = cv2.cvtColor(image_cv, cv2.COLOR_BGR2GRAY)
                self.grayscale = True
            else:
                gray = image_cv
    ...
    def step_17_thresholding(self):
        ...
        try:
            if self.grayscale:
                ...
                # Ensure image is 8-bit single channel
                if len(image_cv.shape) > 2:  # If somehow not grayscale
                ...
                self.report['steps']['17_thresholding'] = {
                    'status': 'skipped',
                    'message': 'Thresholding skipped (color image)'
                }
        except Exception as e:
            self.report['steps']['17_thresholding'] = {
                'status': 'failed',
                'message': f'Thresholding failed: {str(e)}'
            }
            self.report['flags'].append('Thresholding failed')
